...
import com.armdroid.filechooser.UriUtils;
...
public class FileChooser implements EasyPermissions.PermissionCallbacks {
    ...
    private final String universalMimeType = "*/*";
    ...
    public void getFile(OnContentSelectedListener listener) {
        getFile(listener, null, null);
    }

    public void getFile(OnContentSelectedListener listener, @Nullable String dialogText) {
        getFile(listener, dialogText, null);
    }

    public void getFile(OnContentSelectedListener listener, @Nullable String dialogText, String mimeType) {
        String[] permissions = {Manifest.permission.READ_EXTERNAL_STORAGE};
        this.contentListener = listener;
        if (EasyPermissions.hasPermissions(activity, permissions)) {
            selectFileByMimeType(mimeType);
        } else {
            requestPermissions(dialogText == null ? DEFAULT_DIALOG_TEXT : dialogText,
    ...
    @AfterPermissionGranted(GET_FILE_PERMISSION)
    private void selectFileByMimeType(String mimeType) {
        Intent intent = new Intent(Intent.ACTION_GET_CONTENT);
        intent.addCategory(Intent.CATEGORY_OPENABLE);
        intent.setType(mimeType != null ? mimeType : universalMimeType);
        startActivityForResult(intent, GET_FILE_REQUEST_CODE);
    }
...
//        Content content = new Content(path, thumb, uri, fileSize, 0);
